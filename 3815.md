...
::: rm
```
  optional<pair<Token, @_wrap-sender_@>>
    release() && noexcept(is_nothrow_move_constructible_v<@_wrap-sender_@>);
```
:::
::: add
```
...
to `false`, no further effects; otherwise, initializes _`sndr`_ with `other.@_sndr_@`.
:::
...
`false`, no further effects; otherwise, initializes _`sndr`_ with `std::move(*parts.second)`.
:::
...
:::
::: add
[8]{.pnum} _Effects:_ If _`assoc`_ contextually converts to `false` then no effect; otherwise, destroys _`sndr`_.
:::
...
```
  ...
  using assoc_t = typename associate_data_t::@_assoc-t_@;
  using sender_ref_t = typename associate_data_t::@_sender-ref_@;
```
:::
```
...
::: add
```
  (is_rvalue_reference_v<Sndr&&> || is_nothrow_constructible_v<remove_cvref_t<Sndr>, Sndr>) &&
```
:::
```
...
:::
::: add
[6]{.pnum} _Effects:_ Initializes _`alloc`_ with `std::move(alloc)`, _`op`_ with
`connect(std::move(sndr), @_spawn-receiver_@(this))`, and _`assoc`_ with `token.try_associate()`.
:::
...
:::
::: add
[6]{.pnum} _Returns:_ An object `a` of type `simple_counting_scope::assoc` such that `a.@_scope_@` is `this` if
_`count`_ was incremented, `nullptr` otherwise.
:::
...
::: add
__Association [exec.simple.counting.assoc]__
...
namespace std::execution {
  ...
  private:
    using @_handle_@ = // @_exposition only_@
      unique_ptr<simple_counting_scope, decltype([](auto* p) noexcept {
        p->@_disassociate_@();
      })>;

    @_handle_@ @_scope_@; // @_exposition only_@
  };
}
```
...
[1]{.pnum} _Returns:_ `@_scope_@ != nullptr`
